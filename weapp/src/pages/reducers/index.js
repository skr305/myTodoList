    ...
    case "ADDTYPE":
     ...
     { 
      ...
      let newList = Object.assign({}, todoList[typeKey].list)
       newList[todoKey] = {"todoName": todoName, "isOk": false, "typeKey": typeKey, "endTime": formatDateToTime(endTime), typeName: store["todoList"][typeKey].typeName}
         ...
         case "DELTYPE":
           {
              ...
              if(newTodoList[typeKey] !== undefined)
              {
                  ...
                  //保险步骤 有些运行环境下这套代码不知道为什么不能通过reducer来更改store的数据
                  //所以这里采用直接删除的方式来进行
